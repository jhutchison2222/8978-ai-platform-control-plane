...
`src/test-runtime-stores.js` contains deterministic test doubles for Node contract tests. The development Worker now injects separately tested SQLite Durable Object implementations for idempotency leases, owner-decision consumption, and append-only audit chains. Migration-backed, read-only D1 implementations now exist for resource resolution, digest-bound limits, Ed25519 maker/checker identity, required-test evidence, executable rollback evidence, governing Project Knowledge snapshots, Ed25519 owner decisions, standing-state/kill-switch revalidation, authenticated development-activation evidence bundles, and authenticated-write receipt integrity. A separately tested, unwired chain verifier requires both activation-evidence verification paths and re-enforces four-principal independence. The HMAC-authenticated, replay-protected, append-only evidence writer remains unwired, unbound, unseeded, and undeployed. The remaining authoritative adapters are still unavailable, and provider credentials or private signing keys must never be stored in Project Knowledge or evidence payloads.
