...
Status: CONTRACT WITH NON-GOVERNING CANDIDATE — REVIEW COMPLETED WITHOUT FORMAL ACCEPTANCE

This contract defines three honest outcomes for the separately authorized read-only authority-schema verification pass: `VERIFIED`, `STOPPED_NO_QUERY`, and `INCONCLUSIVE_READ_ONLY`. The checked-in candidate records the successful one-attempt execution and the completed Claude review while remaining non-governing and inconclusive because that review did not explicitly accept the exact head.
