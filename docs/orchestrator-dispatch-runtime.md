...
The Workers test configuration injects local Miniflare Workflow and Queue bindings so the real platform-shaped APIs are exercised. The code-only wiring candidate adds the exact reviewed development Workflow and Queue-producer bindings without deploying them. The Worker still reports `ready: false`, keeps `ALLOW_EXTERNAL_WRITES="false"`, and returns `execution_disabled` for every `/v1/actions/execute` request; the Workflow itself also returns `execution_disabled`.
